...
bool checkColi(Dino dino, Enemy enemy) {
    SDL_Rect d_rect = { dino.getPosX() + 10, dino.getPosY() + 10, dino.getRectWidth() - 25, dino.getRectHeight() - 20 };
    SDL_Rect e_rect;
...
void DinoGame::displayGame() {
    ...
    fontGame = TTF_OpenFont("Resource/Font/no_continue.ttf", FONT_SIZE);
    if (fontGame == NULL) {
        ...
        while (isRunning) {
            ...
            while (SDL_PollEvent(&event)) {
                ...
                if (isPauseGame == false) {
                    if (dino.inputAction(event, gJumpMusic)) {
